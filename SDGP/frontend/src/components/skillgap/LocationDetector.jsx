...
export default function LocationDetector({ onLocationDetected }) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 text-sm">
      {/* Status Badge */}
      <div
        className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full font-medium text-xs border ${
          status === 'granted'
      ...
      {showManual && (
        <div className="flex gap-2 mt-2 w-full">
          ...
          <button
            onClick={applyManual}
            className="text-xs bg-purple-600 text-white px-3 rounded-lg hover:bg-purple-700"
          >
            Apply
          </button>
        </div>
