...
export default function PathwayDetailModal({ pathway, careerTitle, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={onClose}>
      ...
      >
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4 bg-gradient-to-r from-purple-50 to-blue-50 border-b border-slate-100">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl bg-purple-100 flex items-center justify-center">
              <BookOpen className="w-4 h-4 text-purple-600" />
            </div>
            <div>
              <p className="font-bold text-slate-800">{pathway.label}</p>
              <p className="text-xs text-slate-500">{careerTitle}</p>
            </div>
          </div>
        ...
        <div className="p-5 space-y-5 max-h-[70vh] overflow-y-auto">
          ...
          {/* Cost Breakdown */}
          <div>
            <h3 className="font-semibold text-slate-700 mb-3 text-sm uppercase tracking-wide">Cost Breakdown</h3>
            <div className="space-y-2">
              {items.map((item, i) => {
                const pct = Math.round((item.value / pathway.cost) * 100);
                return (
                  <div key={i} className="flex items-center gap-3">
                    <span className="text-sm text-slate-600 w-40 flex-shrink-0">{item.label}</span>
                    ...
                    <span className="text-sm font-semibold text-slate-700 w-28 text-right">{LKR(item.value)}</span>
                  </div>
                );
              })}
            </div>
          ...
          {pathway.skillsGained && (
            <div>
              <h3 className="font-semibold text-slate-700 mb-3 text-sm uppercase tracking-wide">Skills You'll Gain</h3>
          ...
          {/* Outcomes */}
          <div>
            <h3 className="font-semibold text-slate-700 mb-3 text-sm uppercase tracking-wide">Key Outcomes</h3>
