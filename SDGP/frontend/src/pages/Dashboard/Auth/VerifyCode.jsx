...
export default function VerifyCode() {
  ...
  // Extract email from query params (passed from ForgotPassword)
  const queryParams = new URLSearchParams(location.search);
  const email = queryParams.get("email");

  useEffect(() => {
    if (!email) {
      setError("No email address provided. Please restart the process.");
    }
  ...
  const handleVerify = async (e) => {
    ...
    try {
      ...
      // Auto redirect to reset-password with the new secure JWT token
      if (data.resetToken) {
        setTimeout(() => {
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-indigo-50 flex items-center justify-center px-4">
      <div className="w-full max-w-md bg-white rounded-2xl shadow-xl border border-slate-100 p-8">
        ...
        <p className="text-sm text-center text-slate-500 mb-6">
          We've sent a 6-digit code to <span className="font-semibold text-slate-700">{email || "your email"}</span>. Enter it below.
        </p>
